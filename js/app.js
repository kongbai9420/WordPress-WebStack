...
(function() {
	var themeInfo = (typeof window.theme !== 'undefined' && window.theme.version) ? window.theme.version : '2026.8.4';
	console.log("\n %c WebStack导航主题 V"+themeInfo+" \n", "color: #ffffff; background: #667eea; padding:5px 0;");
})();
...
;(function($, window, undefined)
{
	...
	$(document).ready(function()
	{
		...
		if($tooltips.length && $tooltips.length <= 40 && !isxs())
		{
...
function ps_init()
{
	if(public_vars.$sidebarMenu && public_vars.$sidebarMenu.length)
	{
		public_vars.$sidebarMenu.find('.sidebar-menu-inner').css({
			overflowY: 'auto',
			overflowX: 'hidden'
		});
	}
...
/* Footer visitor stats heartbeat */
(function ($) {
    'use strict';
    ...
    function getVisitorId() {
        var id = null;
        try {
            id = window.localStorage.getItem('nav_stats_vid_hb');
        } catch (e) { id = null; }
        ...
        try {
            window.localStorage.setItem('nav_stats_vid_hb', out);
        } catch (e) { /* ignore */ }
        return out;
    }
    ...
    function applyValue($el, value) {
        if ($el.length) {
            ...
            $el.text(isNaN(num) ? '--' : num.toLocaleString('zh-CN'));
        }
    }
    ...
    function heartbeat() {
        $.ajax({
            ...
            dataType: 'json',
            data: {
                action: 'nav_stats_heartbeat',
                nonce: window.theme.nav_stats_nonce,
                visitor_id: getVisitorId()
            },
            success: function (res) {
                if (res && res.success && res.data) {
                    applyValue($online, res.data.online);
                    applyValue($today, res.data.today);
                    applyValue($total, res.data.total);
                }
            },
            error: function () {
                // AJAX 不可用时保留服务端初始值，避免覆盖已显示的在线人数。
            }
        });
    }
